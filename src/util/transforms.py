...
def to_float_tensor(vid):
    return torch.from_numpy(vid.astype('float32'))
...
def minmax_normalize_tensor(vid, vmin, vmax, scale=2):
    vid = torch.clip(vid, vmin, vmax)
    vid = vid - vmin
    vid = vid / (vmax - vmin)
...
def log_transform(data, k=1, c=0, log10=True):
    if log10:
...
def log_transform_tensor(data, k=1, c=0, log10=True):
    if log10:
        return (torch.log10(torch.abs(k * data) + c)) * torch.sign(data)
    else:
...
def denormalize(vid, vmin, vmax):
    return vid * vmax + vmin


def exp_transform(data, k=1, c=0, log10=True):
    if log10:
        return (np.power(10, np.abs(data)) - c) * np.sign(data) / k
    else:
        return (np.expm1(np.abs(data)) - c) * np.sign(data) / k

def exp_transform_tensor(data, k=1, c=0, log10=True):
    if log10:
        return (torch.pow(10, torch.abs(data)) - c) * torch.sign(data) / k
    else:
...
def tonumpy_denormalize(vid, vmin, vmax, exp=True, k=1, c=0, scale=2, norm='minmax', log10=True):
    if norm == 'minmax':
        vid = minmax_denormalize(vid.cpu().numpy(), vmin, vmax, scale)
        return exp_transform(vid, k=k, c=c, log10=log10) if exp else vid
    elif norm == 'std':
        vid = denormalize(vid.cpu().numpy(), vmin, vmax)
        return exp_transform(vid, k=k, c=c, log10=log10) if exp else vid
    else:
        return exp_transform(vid.cpu().numpy(), k=k, c=c, log10=log10) if exp else vid.cpu().numpy()
    
def tensor_denormalize(vid, vmin, vmax, exp=True, k=1, c=0, scale=2, norm='minmax', log10=True):
    if norm == 'minmax':
        vid = minmax_denormalize(vid, vmin, vmax, scale)
        return exp_transform_tensor(vid, k=k, c=c, log10=log10) if exp else vid
    elif norm == 'std':
        vid = denormalize(vid, vmin, vmax)
        return exp_transform_tensor(vid, k=k, c=c, log10=log10) if exp else vid
    else:
        return exp_transform_tensor(vid, k=k, c=c, log10=log10) if exp else vid


def square_label(vid):
    return vid ** 2


def sample_to_whole(vid, keep_step=1):
    B, H, W = vid.shape
    assert keep_step <= H
    
    original_H = B + H - 1
    whole = torch.zeros(original_H, W).to(vid.device)
    add_num = torch.zeros(original_H, 1).to(vid.device)
    ...
    for i in range(vid.shape[0]):
        ...
        if i == 0:
            ...
            part = sample[-keep_step:]
            j = i + H
            whole[j-keep_step: j] += part
            add_num[j-keep_step: j] += 1
...
class LogTransform_tensor(object):
    def __init__(self, k=1, c=0):
        self.k = k
        self.c = c
...
class MinMaxNormalize_tensor(object):
    def __init__(self, datamin, datamax, scale=2):
        self.datamin = datamin
        self.datamax = datamax
        self.scale = scale
    ...
    def __call__(self, vid):
        return minmax_normalize_tensor(vid, self.datamin, self.datamax, self.scale)


class DownSample(object):
    def __call__(self, vid):
        return down_sample(vid)
...
class StackPCD(object):
    ...
    def __call__(self, data):
        ...
        secondary_pc = secondary_pc[:, np.newaxis, np.newaxis]
